...
<div style="width: 100%">
    ...
    <!--清除浮动-->
    <!--4. 第三部分是注册部分-->
    <div class="j"  style="background: url(${pageContext.request.contextPath }/img/2.jpg);height: 700px;background-size:100% 100%;">
        <div style="position:absolute;top:200px;left:350px;border: 5px solid darkgray;width: 50%;height: 50%;background-color: white;opacity: 0.9">
            <form action="${pageContext.request.contextPath }/user_editUser.action" method="post">
            <table width="60%" align="center"style="margin-top: 8%;margin-left: 28%;">
                <tr><td>用户名:</td><td><input type="hidden" name="user_id" value="<s:property value="#session.exisUser.user_id"/>"><input name="user_account" type="text" value="<s:property value="#session.exisUser.user_account"/>"/></td></tr>
                <tr><td>email:</td><td><input name="user_email" type="email" value="<s:property value="#session.exisUser.user_email"/>"/></td></tr>
                <tr><td>姓名:</td><td><input name="user_name" type="text" value="<s:property value="#session.exisUser.user_name"/>"/></td></tr>
                <tr><td style="position: absolute;top: 132px;">性别:</td>
                    <s:radio name="user_gender" list="#{'0':'男','1':'女'}" value="#session.exisUser.user_gender"/>
                </tr>
                <tr><td>出生日期:</td><td><input name="user_birthday" type="date" value="<s:date name="#session.exisUser.user_birthday" format="yyyy-MM-dd" />"/></td></tr>
            </table>
            <div style="position:relative;left: 42%; margin-top:30px">
                <input type="submit" value="保存" style="margin-right:30px; "/>
                <input type="button" value="取消" onclick="javascrtpt:window.location.href='${pageContext.request.contextPath }/user/information.jsp'"/>
            </div>
            </form>
        </div>
    </div>
